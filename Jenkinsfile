pipeline { 
agent any //Any agent, including master agent 
stages { 
stage('First stage'){ 
steps{ 
...
-Dsonar.projectName=${project_key} -Dsonar.java.binaries=build/classes" 
} 
} 
} 
} 
...
stage('Deploy DEV') { 
steps{ 
sh 'docker ps -f name=api-container-dev -q | xargs --no-run-if-empty docker rm -f' sh "docker run -d --name api-container-dev -p 8080:8080 
api-demo:v${env.BUILD_NUMBER}" 
} 
} 
stage('Api Test'){ 
steps{ 
